...
test('retourne une URL Unsplash pour "attiéké" seul (sans poisson)', () => {
  ...
  const result = getArticleImage(article);
  assert.ok(result.startsWith(UNSPLASH_BASE), 'doit retourner une URL Unsplash');
  assert.ok(result.includes('1665332195309-9d75071138f0'), 'attiéké seul doit correspondre à l\'image dédiée');
});
...
test('retourne une URL Unsplash pour "kedjenou poulet" (plat ivoirien)', () => {
  ...
  const result = getArticleImage(article);
  assert.ok(result.startsWith(UNSPLASH_BASE), 'doit retourner une URL Unsplash');
  assert.ok(result.includes('1664992960082-0ea299a9c53e'), 'kedjenou doit correspondre à l\'image plat ivoirien');
});
...
test('retourne une URL Unsplash pour "alloco" (banane plantain frite)', () => {
  ...
  const result = getArticleImage(article);
  assert.ok(result.startsWith(UNSPLASH_BASE), 'doit retourner une URL Unsplash');
  assert.ok(result.includes('1603496987674-79600a000f55'), 'alloco doit correspondre à l\'image plat doré');
});

  ...
  return (
    <div className="min-h-screen bg-white p-4 md:p-6">
      ...
      <header className="mb-8">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex gap-3">
              <div className="bg-white rounded-lg px-5 py-3 shadow-sm border border-slate-100 flex items-center gap-3">
              ...
              {urgentOrders > 0 && (
                <div className="bg-white rounded-lg px-5 py-3 shadow-sm border border-red-100 flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <input
                  ...
                  className="pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-lg w-72 focus:outline-none focus:border-amber-400 text-sm"
                />
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 hover:border-slate-300 rounded-lg text-sm font-medium text-slate-700 transition-all active:scale-95"
              >
        ...
        {lastRealtimeEvent && (
          <div className="mt-4 inline-flex items-center gap-2.5 px-5 py-2 bg-amber-50 text-amber-700 text-sm font-medium rounded-lg border border-amber-100 shadow-sm">
      ...
      {error && (
        <div className="mb-6 flex items-center gap-3 px-6 py-4 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        {columnsData.map((col) => (
          <div key={col.key} className="flex flex-col bg-white/70 backdrop-blur-xl rounded-lg border border-slate-100 shadow-xl overflow-hidden h-full">
            {/* Column Header */}
            <div className="px-6 py-5 border-b border-slate-100 bg-white flex items-center justify-between sticky top-0 z-10">
              ...
              <div className="bg-slate-100 text-slate-600 text-sm font-bold px-4 py-1.5 rounded-lg">
                {col.orders.length}
              </div>
            </div>
            ...
            <div className="flex-1 p-4 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent min-h-[500px]">
              {col.orders.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full py-16 text-center">
                  <div className="w-20 h-20 bg-slate-100 rounded-lg flex items-center justify-center mb-6">
...
function OrderCard({
  ...
  return (
    <div className={`group bg-white rounded-lg border shadow-sm overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-0.5 ${isUrgent || isOverdue ? 'border-red-300 ring-1 ring-red-200' : isWarning ? 'border-amber-300' : 'border-slate-200'}`}>
      {/* Header */}
      <div className="px-6 py-5 border-b border-slate-100 flex items-start justify-between bg-gradient-to-r from-white to-slate-50">
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-lg flex items-center justify-center font-bold text-xl shadow-inner transition-colors ${isOverdue ? 'bg-red-600 text-white' : isUrgent ? 'bg-red-100 text-red-700' : 'bg-gradient-to-br from-amber-100 to-orange-100 text-amber-700'}`}>
            #{order.numero.slice(-4)}
          </div>

          <div>
            <div className={`inline-flex items-center px-3 py-1 text-xs font-bold rounded-lg border ${STATUS_COLORS[order.statut] || 'bg-slate-100 text-slate-600'}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <select
                  ...
                  className="px-4 py-3 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-amber-400"
                >
              ...
              <button
                ...
                className="w-full py-3.5 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 disabled:from-slate-400 disabled:to-slate-500 text-white font-bold rounded-lg text-sm transition-all active:scale-[0.985] flex items-center justify-center gap-2 shadow-md"
              >
      ...
      <div className="p-6 space-y-3">
          ...
          <div className="space-y-2">
            {nextStatuses.map((nextStatus) => (
              <button
                ...
                className="w-full py-4 bg-slate-900 hover:bg-black text-white rounded-lg font-semibold text-sm flex items-center justify-center gap-2 transition-all active:scale-[0.985] disabled:bg-slate-400"
              >
        ...
        {showDispatch && (
          <button
            ...
            className="w-full py-4 border-2 border-amber-600 text-amber-700 hover:bg-amber-50 font-semibold rounded-lg flex items-center justify-center gap-2 text-sm transition-all"
          >

...
export class WipeNonAdminUsers1785486196118 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DELETE FROM password_resets WHERE "userId" IN ${nonAdmin}`,
    );

    // Appartenance à une équipe B2B : table de jonction, vide de sens
    // sans l'utilisateur.
    await queryRunner.query(
      `DELETE FROM b2b_team_members WHERE user_id IN ${nonAdmin}`,
    );
    ...
    await queryRunner.query(
      `UPDATE b2b_teams SET created_by = NULL WHERE created_by IN ${nonAdmin}`,
    );
    await queryRunner.query(
      `UPDATE b2b_invoices SET b2b_client_id = NULL WHERE b2b_client_id IN ${nonAdmin}`,
    );
    await queryRunner.query(
      `UPDATE b2b_bulk_orders SET created_by = NULL WHERE created_by IN ${nonAdmin}`,
    );
    await queryRunner.query(
      `UPDATE commandes SET "clientId" = NULL WHERE "clientId" IN ${nonAdmin}`,
    );
    await queryRunner.query(
      `UPDATE avis_commandes SET "clientId" = NULL WHERE "clientId" IN ${nonAdmin}`,
    );
